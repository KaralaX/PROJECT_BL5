...
public class TicketServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        OrderDetailDAO ordet = new OrderDetailDAO();
        UserDAO ud = new UserDAO();
        HttpSession session = request.getSession();
        String sort = request.getParameter("sort");
        ArrayList<Ord> data = new ArrayList<>();
        User u = ud.findByUserName((String) session.getAttribute("username"));
        if(sort==null){
            data = ordet.getOrderDetail(u.getUserid(), true);
            ...
            if(Integer.parseInt(sort) == 1){
                ...
                data = ordet.getOrderDetail(u.getUserid(), false);
